...
void draw_features(cv::Mat& src, const std::vector<Feature>& features)
{
    for (int i = 0; i < features.size(); i ++) {
        cv::circle(src, cv::Point(features[i].x, features[i].y), 4, 122, 2);
        //src.at<uchar>() = 122;
    }
}
...
std::vector<cv::Mat> draw_pyramid(cv::Mat& src, const Pyramid& pyramid)
{
    ...
    for (int i = 0; i < pyramid.size(); i ++) {
        ...
        for (int i = 0; i < pattern.m_features.size(); i ++) {
            ...
            cv::circle(src_draw, cv::Point(pattern.m_features[i].x + base_x, pattern.m_features[i].y + base_y), 4, 122, 2);
        }
